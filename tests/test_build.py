...
def test_run_writes_outputs(tmp_path):
    (tmp_path / "models.yaml").write_text(
        "models:\n  - id: a-model\n    name: A\n    vendor: V\n"
        "    aliases: {livebench: ['a-raw']}\n"
    )
    (tmp_path / "sources.yaml").write_text("sources: []\n")
    fake_results = {
        "livebench": {
            "entries": [{"raw_name": "a-raw", "metrics": {"average": 50.0}}],
    ...
    assert latest["models"]["a-model"]["scores"]["livebench"]["average"] == 50.0
    ...
    assert trends["a-model"]["livebench"]["average"] == [["2026-07-03", 50.0]]
